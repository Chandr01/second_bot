...
#     conn = sqlite3.connect("base.db")
#     cursor = conn.cursor()
#     table = cursor.execute('SELECT id FROM main WHERE id =\'{}\''.format(chat_id)).fetchall()
#     conn.close()
#     print(table)
#     return table
...
def textMessage(bot, update):
    ...
    if bool(list(set(text_away) & set(message.lower().split(' ')))):
        ...
        if get_status(chat_id):
            ...
            text_save = u'{}, попробовал уйти когда и так уже отсутствовал'.format(user_name_id)
            save_file(text_save)
            ...
            text_save = u'{}, ушел'.format(user_name_id)
            save_file(text_save)
            # name = get_name(chat_id)[0][1]
            text = '{} ушел(а)'.format(user_name_id)
        ...
        if not get_status(chat_id):
            ...
            text_save = u'{}, попробовал прийти пока не ушел'.format(user_name_id)
            save_file(text_save)
            bot.send_message(chat_id=update.message.chat_id, text=text)
            ...
            text_save = u'{}, вернулся (отсутствовал - {})'.format(user_name_id, away_time)
            save_file(text_save)
            text = '{} пришел(а), его(её) не было {} мин.'.format(user_name_id, minutes)
            bot.send_message(chat_id=update.message.chat_id, text=text)
